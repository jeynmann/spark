...
/**
 * Usage: GroupByTestSpill [numMappers] [numKVPairs] [KeySize] [numReducers] [seed] [policy] [randMax] [numSpill] [sizeSpill]
 */
object GroupByTestSpill {
  def main(args: Array[String]) {
    val spark = SparkSession
      .builder
      .appName(s"GroupBy Test Spill ${System.currentTimeMillis}")
      .getOrCreate()
    ...
    val numSpill = if (args.length > 7) args(7).toInt else 1
    val sizeSpill = if (args.length > 8) args(8).toLong else valSize

    val numKVPerSpill = ((sizeSpill + valSize - 1) / valSize).toInt.max(1)
    val numSpillPerMap = ((numKVPairs + numKVPerSpill - 1) / numKVPerSpill).min(numSpill)
    val spillKeyGen = new Random(seed - 1)
    val spillKeys = new Array[Int](numSpillPerMap)
    for (i <- 0 until spillKeys.length) {
      spillKeys(i) = spillKeyGen.nextInt(randMax)
    }
    ...
    val pairs0 = spark.sparkContext.parallelize(0 until numMappers, numMappers).flatMap { p =>
      ...
      val numKVSpill = numKVPerSpill * numSpill
      for (i <- 0 until numKVSpill.min(numKVPairs)) {
        val spillKey = spillKeys(i % numSpillPerMap)
        arr1(i) = (spillKey, genVal())
      }

      for (i <- numKVSpill until numKVPairs) {
