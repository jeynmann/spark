...
/**
 * Usage: GroupByTest2ndSeed [numMappers] [numKVPairs] [KeySize] [numReducers]
 */
object GroupByTest2ndSeed {
  def main(args: Array[String]) {
    val spark = SparkSession
      .builder
      .appName("GroupBy Test 2nd Order Seed")
      .getOrCreate()
    ...
    val seedSeq = new Array[Long](numMappers)
    for (p <- 0 until numMappers) {
      seedSeq(p) = seedGen.nextLong
    }

    val pairs0 = spark.sparkContext.parallelize(0 until numMappers, numMappers).flatMap { p =>
      val ranGen = new Random(seedSeq(p))
    ...
    println(s"<zzh> persist=$policy, seed=$seed, next=${seedSeq(0)}")
